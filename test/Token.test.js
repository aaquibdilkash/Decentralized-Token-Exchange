...
contract("Token", ([deployer, receiver, exchange]) => {
  const name = "Decentralized Token";
  const symbol = "DZD";
  const decimals = "18";
  ...
  describe("delgated token transfers", () => {
    ...
    describe("failure", async () => {
      it("rejects insufficient amount", async () => {
        let invalidAmount;
        invalidAmount = tokens(100000000); // 100 million - attempts to transfer too many tokens
        await token
        ...
        invalidAmount = tokens(10); // recipient has no tokens
        await token
          .transferFrom(deployer, receiver, invalidAmount, { from: receiver })
          .should.be.rejectedWith(EVM_REVERT);
      });
      it("rejects invalid recipient", async () => {
